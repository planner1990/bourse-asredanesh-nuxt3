    ...
    <ada-tabs v-model="tab" class="tabs">
      <ada-tab :model="1">
        <form class="frm">
          ...
          <div class="tw-justify-between">
            <ada-currency-input :label="$t('oms.count')" v-model="quantity" :id="`buyCountInputText-${insId}`"
              ...
              :max="!!active ? active.maxQuantityPerOrder || null : null" :nameInput="tab === '1' ? 'focus' : ''">
              <template #append>
                ...
                <div>
                  <ada-btn @click.stop="activeCalculator = !activeCalculator" :width="24" :height="24" tabindex="-1"
                  ...
                  <div class="calculator-container" v-if="activeCalculator"
                    ...
                    <ada-currency-input :label="$t('instrument.wholePrice')" class="tw-mb-2 tw-text-gray3"
                      placeholder="0" v-model="wholePrice">
                    </ada-currency-input>
                    <ada-currency-input class="tw-mb-2 tw-text-gray3" :label="$t('instrument.price')"
          ...
          <div class="tw-justify-between">
            <ada-currency-input :label="$t('oms.price')" v-model="enteredPrice" activeBorder :readonly="priceLock"
          ...
          <div class="tw-justify-between">
            <wealth-validity v-model:validityDate="validityDate" v-model:validityType="validityType" height="24px"
              class="tw-my-1 inputColor" :label="$t('accounting.account.credit')" tabindex="-1" :class="{
          ...
          <div class="tw-justify-between tw-col-span-2">
            <oms-buy-sell-card-percent v-model="order.discloseQuantity" :label="$t('oms.view-count')" height="31px"
              class="tw-flex tw-flex-grow inputColor" :min="30" :max="100" :total="quantity" :amount="100"
              tabindex="-1">
            </oms-buy-sell-card-percent>
          </div>
          <div class="tw-justify-between">
            <ada-input :label="$t('wealth.order.creditPercent')" class="tw-h-[24px] inputColor" type="number" :min="0"
      ...
      <ada-tab :model="2">
        <form class="frm">
          ...
          <div class="tw-justify-between">
            <ada-currency-input :label="$t('oms.count')" v-model="quantity" :id="`sellCountInputText-${insId}`"
              :readonly="countLock" class="inputColor" :class="{
                inValid: errors?.quantity,
                valid: quantityMeta?.valid && quantityMeta?.validated,
              }" activeBorder :min="!!active ? active.minQuantityPerOrder : 1"
              :max="!!active ? active.maxQuantityPerOrder || null : null" :nameInput="tab === '2' ? 'focus' : ''">
              <template #append>
                <ada-btn :class="[countLock ? 'active' : '']" :width="24" :height="24" @click="toggleCountLock" icon
                  tabindex="-1">
                  <ada-icon color="primary">isax-lock-1</ada-icon>
                </ada-btn>
                <ada-menu :active="activeCalculatorSell" :mWidth="180" :mLeft="-11.5" :mTop="30">
                  <template #activator>
                    <ada-btn @click.stop="activeCalculatorSell = !activeCalculatorSell" :width="24" :height="24"
                      tabindex="-1" :class="[activeCalculatorSell ? 'tw-bg-primary' : null]">
                      <ada-icon :class="[activeCalculatorSell ? 'tw-text-white' : null]">isax-calculator
                      </ada-icon>
                    </ada-btn>
                  </template>
                  <template #items>
                    <div class="calculate" v-ada-click-outside="() => (activeCalculatorSell = false)">
                      <ada-currency-input :label="$t('instrument.wholePrice')" placeholder="0" v-model="wholePrice">
                      </ada-currency-input>
          ...
          <div class="tw-justify-between">
            <ada-currency-input :label="$t('oms.price')" v-model="enteredPrice" :readonly="priceLock" class="inputColor"
              :class="{
                inValid: errors?.enteredPrice,
                valid: enteredPriceMeta?.valid && enteredPriceMeta?.validated,
          ...
          <div class="tw-justify-between tw-col-span-2">
            <oms-buy-sell-card-percent v-model="order.discloseQuantity" :label="$t('oms.view-count')" height="31px"
              class="tw-flex tw-flex-grow inputColor" :min="30" :total="quantity" :amount="100" tabindex="-1">
            </oms-buy-sell-card-percent>
          </div>
          <div class="tw-justify-between">
            <ada-input :label="$t('oms.stockPercent')" type="number" class="tw-h-[24px] inputColor" :min="0" :max="100"
