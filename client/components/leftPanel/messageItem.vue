...
<template>
    <div class="message" v-bind="$attrs" @click="show_message()">
        <ada-btn color="transparent" class="template">
            <ada-icon :size="26" v-if="type == MessageOrigin.rlc" class="tw-text-primary">lotfi-sms</ada-icon>
            <ada-icon :size="26" v-else-if="props.type == MessageOrigin.codal" class="tw-text-success">isax-document-text-outline
            </ada-icon>
            <ada-icon :size="26" v-else-if="props.type == MessageOrigin.tedan" class="tw-text-error">isax-messages-3-outline
            </ada-icon>
            <ada-icon :size="26" v-else class="tw-text-info">isax-note-favorite-outline</ada-icon>
